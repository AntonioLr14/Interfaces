...
import Vista.doctor.ayudas.Mostrar_ayuda;
...
public class Medico extends JFrame {
	...
	public Medico(String id) {
		...
		this.boton_ayuda.setIcon(new ImageIcon(Medico.class.getResource("/Vista/imagenes/interrogacion.png")));
		...
		this.panel.add(this.boton_ayuda);
		...
		ActionListener visualizar_agenda = (event) -> {
			...
			try {
				this.panelprueba = new Visualizar_Agenda_M(this.id);
			}
			catch (Exception error) {
				error.printStackTrace();
			}

package Vista.administrador.ayudas;
...
public class Mostrar_ayuda extends JFrame{
	...
	public Mostrar_ayuda() {
		...
		DefaultMutableTreeNode administrador = new DefaultMutableTreeNode();
		administrador.setUserObject("administrador");
		
		DefaultTreeCellRenderer nodos = new DefaultTreeCellRenderer();
		nodos.setBackgroundNonSelectionColor(null);

		this.indice = new JTree(administrador);
		...
		try {
			...
			DirectoryStream<Path> directorios = Files.newDirectoryStream(Path.of("src/Vista/administrador/ayudas/html/"), "*_*");
		...
		this.indice.addTreeSelectionListener( (event) -> {
			...
			try {
				...
				if (nodo.isLeaf()) {
					...
					String contenido = Files.readString(Path.of("src/Vista/administrador/ayudas/html/" + directorio + "/" + archivo + ".html"));
		...
		setTitle("Ayuda del administrador");
